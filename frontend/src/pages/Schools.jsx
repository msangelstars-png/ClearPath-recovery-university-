        ...
        {schools.map((school) => (
          <article key={school.id} className="overflow-hidden rounded-2xl border border-brand-border bg-white transition-transform duration-300 hover:-translate-y-1 hover:shadow-md" data-testid={`school-card-${school.id}`}>
            ...
            <div className="p-5">
              ...
              <Button onClick={() => enrollSchool(school.id)} disabled={school.enrolled} data-testid={`school-enroll-button-${school.id}`} className="mt-5 w-full rounded-full bg-brand-primary text-white hover:bg-brand-primaryHover">{school.enrolled ? "Enrolled" : "Enroll in school"}</Button>
            </div>
          </article>
        ))}
      </div>
