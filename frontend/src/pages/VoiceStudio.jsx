...
export default function VoiceStudio() {
  ...
  useEffect(() => { platformApi.voiceProfessors().then(({ data }) => { setProfessors(data.professors); setActive(data.professors[0]); }); }, []);
  const start = async () => { if (!active) return; const { data } = await platformApi.voiceSession({ professor_id: active.id, mode, language: "en" }); setSession(data.session); };
  const speak = () => {
